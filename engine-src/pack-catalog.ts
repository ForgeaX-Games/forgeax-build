...
interface ExternalAssetMetaJson {
  ...
  readonly importer: 'image' | 'gltf' | 'audio' | 'font' | 'ui';
...
// for game roots (the play-runtime global pluginPack scans ONLY shared-assets),
// so a raw-source ui row would 404 the cook and surface to the game as
// `asset-not-imported` (this was the HUD/settings UI PLAY bug). This mirrors the
...
// are byte-identical to the raw sources, so the emitted pack is identical to the
// engine `POST /__import` ui arm output. Doing it dependency-free keeps this
// vite-config-time module from importing the engine UI importer (with its
// css-tree/parse5 validation stack) just to no-op. UIs that DO reference image
// companions cannot be served by this per-game dev route anyway (it serves no
// `/__ui/*` bodies) and must be pre-imported at build time.
//
...
async function cookUiPack(
  ...
  // source is newer (edit-through), so the pack-index HTTP hot path does not
  // re-cook + re-write on every request.
  try {
  ...
  if (meta.importer === 'image') {
    ...
    for (const sub of meta.subAssets) {
      if (sub.kind === 'equirect') {
        ...
        out.push({ guid: sub.guid, relativeUrl: normalizedUrl, kind: 'cube-texture', sourcePath: sourceRel, name: subName(sub), metadata: cubeMetadata });
      }
    }
  }
