...
export default defineConfig({
  ...
  server: {
    ...
    strictPort: true,
    open: false,
    // This engine root watches the SYMLINKED `.forgeax/games` tree (run.sh /
    // lib.rs link engine-src/.forgeax → the instance's real .forgeax). macOS
    // FSEvents is UNRELIABLE through symlinks, so game-file HMR + the
    // gameWatcher restart plugin below depend on polling — keep it ON here.
    // interval 1000ms (was 300) cuts the idle re-stat rate to 1/3.
    watch: {
      usePolling: true,
      interval: 1000,
      ignored: [
    ...
    // FORGEAX_INTERFACE_PORT for exactly this case. Local dev leaves both
    // unset; the default 18920 works because the browser talks to vite
    // directly.
    hmr: {
      clientPort: Number(
